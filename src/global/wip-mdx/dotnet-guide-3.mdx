...
Last week, we introduced how to install .NET locally, how to create your first project, as well as making some first changes on it
...
.NET projects. Therefore, for the next few episodes, we will be extensively covering everything you need to know to use this language effectively, from
...
If you are coming from a Java or C/C++ background, learning C# will be much easier than when you first learnt those languages, given the vast similaries
of syntax and concepts. If you are coming from Python however, be prepared for some concepts you have never seen before, especially those around data
types. I will try to accomodate for both types of users by explaining these concepts as detailed as I can.

So, without further ado, let's get into the basics of C#.

# A General Guide to C#

C# (pronounced "C-sharp" (not "C-hash") (nor "C-pound")) is an object-oriented high-level programming language developed by Microsoft. It is designed to be simple,
powerful, type-safe, and versatile. While the language has "C" in its name, it is much closer to Java than C/C++ conceptually and functionally.
...
# Basic Inputs, Basics Outputs

Any languages need the ability to communicate with the user. In the <Redirect name="previous" url="/blogs/dotnet-guide-2"/> episode, we wrote a
greeting program that asks for your name and print out a greeting for you. Let's take a look at the program once again:
...
`<type>` is the type of the variable. There are two types of variables in C#, value types and reference types.

Value types store the actual value of the data directly. Value types are often small, and so can be passed around easily. There are many value type
variables, but the most common ones are:
...
often quite big, in cases where you are duplicating the data (perhaps passing it around as arguments), just sending the reference is much faster and
uses less memory than copying everything. Much like value types, there are many reference types variables as well, such as:
...
Reference types are also special because they can have a `null` value. `null` essentially mean the reference is pointing to nothing, think of it as an
address to an empty space. Since `null` is the lack of data, we sometimes want to think about whether a variable is nullable or not. Adding a question
mark (e.g. `string?`) after the type of a reference-type variable marks that it is nullable.

Take for example a variable we created above:
...
Sometimes, we need change the type of the variable from one to another. For instance, user inputs via `Console.ReadLine()` are returned as strings,
...
# My Project Is Dying. Quick, Send In The Operators!

Finally, C# has a variety of operators to helps these variables interact with each other. These includes arithmetic, bitwise, assignment, comparison,
and logical.
